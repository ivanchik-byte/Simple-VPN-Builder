...
> **Статус: активная бета.** Готовлю первый релиз `v0.1.0`. Код уже работает у меня на боевых серверах, но API и структура конфигов еще могут точечно меняться.
...
Чтобы привязать новый Linux-сервер к кластеру, скопируйте готовую команду прямо из веб-интерфейса и запустите ее на сервере:
...
Скрипт сам поставит `vpnbuilder-agent`, сгенерирует ключи, получит подписанный mTLS-сертификат у встроенного CA панели, пропишет правила в `nftables` и сразу поднимет постоянную связь с панелью.
