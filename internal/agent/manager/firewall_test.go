...
func TestNftablesFirewall_BuildRuleset(t *testing.T) {
	...
	assert.Contains(t, rules, "ip saddr 10.8.0.0/24 oifname != \"wg0\" masquerade")
	assert.Contains(t, rules, "ip6 saddr fd00::/64 oifname != \"wg0\" masquerade")
	assert.Contains(t, rules, "ip protocol icmp icmp type destination-unreachable accept")
}
...
func TestNftablesFirewall_ApplyAndClear_Mock(t *testing.T) {
	...
	mockExec := func(ctx context.Context, name string, arg ...string) *exec.Cmd {
		...
		// Use echo as a no-op command that succeeds
